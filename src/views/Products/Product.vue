          ...
          <v-col cols="12" sm="7">
            <v-card flat>
              ...
              <v-card-title class="text-body-1"
                >VARIATION AVAILABLE</v-card-title
              >
              <v-card-text>
                <v-btn
                  @click="clearVariations"
                  v-if="selectedVariation.length >= 1"
                  icon
                  size="small"
                  flat
                >
                ...
                <v-btn
                  ...
                  :key="i"
                  @click="selectVariation(variation, i)"
                  height="50"
                  :class="
                    selectedVariation.includes(variation) ? 'bg-blue' : ''
                  "
                  class="ma-2"
                  :variant="
                    selectedVariation.includes(variation) ? 'tonal' : 'outlined'
                  "
                  :color="
                    selectedVariation.includes(variation) ? 'white' : 'blue'
                  "
                  rounded="lg"
                  >{{ variation }}</v-btn
                >
                  ...
                  <v-dialog activator="parent" v-model="dialog" width="540">
                    <v-card rounded="xl">
                      <v-toolbar color="transparent" density="compact">
                        <v-toolbar-title
                          >Please select a variation</v-toolbar-title
                        >
                        <v-spacer />
                      ...
                      <v-card-text>
                        ...
                        <v-btn
                          ...
                          height="50"
                          :class="
                            selectedVariation.includes(variation)
                              ? 'bg-blue'
                              : ''
                          "
                          class="ma-2"
                      ...
                      <v-card-actions>
                        ...
                        <v-btn
                          ...
                          class="text-capitalize"
                          style="flex: 1"
                          >Add to cart and checkout</v-btn
                        >
                      </v-card-actions>
                    </v-card>
                  </v-dialog>
    ...
    <v-row class="mt-4">
      <v-col cols="12">
        <v-card rounded="xl">
          ...
          <v-card-text>{{
            product?.condition == "" ? "N/A" : product?.condition
          }}</v-card-text>
          ...
          <v-card-text>
            <v-chip
              v-for="(option, i) in product?.shippingOptions"
              :key="i"
