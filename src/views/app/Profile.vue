            ...
            <v-col cols="12" sm="3">
                <v-card rounded="lg" elevation="5">
                    <v-card-text class="text-center">
                        ...
                        <v-btn size="x-small" rounded="lg" color="blue-lighten-4" class="mt-2 text-capitalize text-blue"
                            flat>
                            Update profile
                            ...
                            <v-dialog activator="parent" v-model="profileUpdate.dialog" width="300">
                                <v-card rounded="xl">
                                    ...
                                    <v-card-text>
                                        <v-text-field v-model="profileUpdate.name" label="Name" variant="underlined"
                                            color="blue" />
                                        <v-text-field v-model="profileUpdate.phone" label="Phone" variant="underlined"
                                            color="blue" />

                                        <v-textarea v-model="profileUpdate.address" label="Address" variant="underlined"
                                            color="blue" />
                                    </v-card-text>

                                    <v-card-actions>
                                        <v-btn :loading="profileUpdate.loading" @click="profileUpdate.update" block
                                            color="blue" class="text-capitalize">Update Profile</v-btn>
                                    </v-card-actions>
                                </v-card>
                            </v-dialog>
                        </v-btn>
                    ...
                    <v-card-subtitle v-if="profile.user?.phone != undefined"
                        class="font-weight-bold">Phone</v-card-subtitle>
                    <v-card-text v-if="profile.user?.phone != undefined" class="pt-0">
                        {{ profile.user?.phone }}
                    </v-card-text>
                    <v-card-subtitle v-if="profile.user?.address != undefined"
                        class="font-weight-bold">Address</v-card-subtitle>
